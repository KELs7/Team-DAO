...
export class TransactionBuilder {
  ...
  addBatchNonce = async (nonceResult) => {
    ...
    if (nonceResult.hosts[0]) {
      ...
      if (this.networkInfo.type == "localhost") {
        this.networkInfo["hosts"] = ["http://0.0.0.0:3737"];
      } else if (this.networkInfo.type == "testnet"){
  ...
  //depracated: not used anymore since results will be returned directly
  //then dev will decide what to with it (check utils.mjs, sendTransaction function)
